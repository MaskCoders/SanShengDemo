 ...
 */
public class SupplementReadMeter extends BaseActivity implements IServiceHandlerCallback,
...
//                break;
            case R.id.meter_address:
                Intent intent = new Intent(SupplementReadMeter.this, MSocketServer.class);
                startService(intent);
                break;
            case R.id.conn_type:
                intent = new Intent(SupplementReadMeter.this, MSocketServer.class);
...
//                mClientManager.createClient(null, -100);
    ...
    private void showWaySelectMeterDialog() {
        WaySelectMeterDialog waySelectMeterDialog = new WaySelectMeterDialog(SupplementReadMeter.this);
        waySelectMeterDialog.show(getFragmentManager(), "way_select_meter");
    }

    private void showConnectTypeDialog() {
        ConnectTypeDialog connectTypeDialog = new ConnectTypeDialog(SupplementReadMeter.this);
    ...
    private HashMap<Integer, String> getSelectProject() {
        String result = EquipmentPreference.getPreferences(SupplementReadMeter.this).getSelectedCollectTest();
        Log.e("ssg", "result = " + result);
        if (TextUtils.isEmpty(result)) {
            result = "[\"0\",\"1\",\"2\"]";
        }
